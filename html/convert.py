...
exps_post = [
    ...
    (
        ...
        r"\\begin{eqnarray}\1\\end{eqnarray}",
    ),
    (r"\\footnote\{[\s\S]*?\}", ""),
    (r"\\begin\{itemize\}", "<ul>"),
    (r"\\end\{itemize\}", "</ul>"),
...
def convertFile(path):
    ...
    for cite in cites:
        rep = "".join(
            [
                '[<a href="../bib.htm#%s">%s</a>]' % (c.strip(), c.strip())
                for c in cite.split(",")
            ]
        )
    ...
    for img in imgs:
        ...
        try:
            ...
            size = 100
        s = s.replace(
            img, r"<img src='../img/%s%s' width='%s'>" % (path, ext, str(size) + "%")
        )
    ...
    for table in tables:
        ...
        try:
            replace = tableshtml[
                tablelabel
            ] + r"<figcaption>%s</figcaption>$$\label{%s}$$" % (caption, tablelabel)
    ...
    if authors:
        ...
        s = s.replace("[AUTHOR]", "")
    with open(
        os.path.join("libro/chapters", name + ".html"), "w", encoding="utf-8"
    ) as f:
        f.write(s)
...
def convertImages():
    ...
    for f in find_files("../latex", "*.pdf"):
        ...
        dest = os.path.abspath("img/%s.png" % dest)
        commands = [r"inkscape", "--export-png=" + dest, os.path.abspath(f)]
        lines.append(" ".join(commands))
    with open("convertimages.bat", "w") as f:
        f.write("\n".join(lines))
...
if __name__ == "__main__":
    ...
    for path in glob.glob("libro/chapters/*.html"):
        ...
        for ref in refs:
            try:
                s = s.replace(
                    r"\ref{%s}" % ref,
                    '<a href="%s.html#%s">%s</a>' % (labelrefs[ref], ref, ref),
                )
            except KeyError:
                pass
                # print ref
