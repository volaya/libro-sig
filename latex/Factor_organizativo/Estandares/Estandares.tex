...
\begin{itemize}
	...
	\item { Gratuidad}. Implementar un estándar es gratuito, sin necesidad de pagar, como en el caso de una patente. Los organismos que generan los estándares pueden cobrar una cierta cantidad por acceder a la definición de los estándares, con objeto de financiar así la labor que desarrollan, y también pueden cobrar por emitir certificados de que un determinado producto o servicio se ha desarrollado de acuerdo con el estándar.
	\item { Discriminación}. Los estándares abiertos y las organizaciones que los desarrollan no favorecen de ningún modo a uno u otro implementador sobre los restantes.
	
	\item { Extensión o creación de subconjuntos de un estándar}. Los estándares abiertos pueden ser extendidos o bien presentados como subconjuntos del estándar original.
	\item { Prácticas predatorias}. Los estándares abiertos pueden tener licencias que requieran a todo aquel que desarrolle una extensión de dicho estándar la publicación de información acerca de esa extensión, y el establecimiento de una licencia dada para todos aquellos que creen, distribuyan y vendan \emph{software} compatible con ella. Un estándar abierto no puede prohibir de otro modo el desarrollo de extensiones.
\end{itemize}
...
\begin{itemize}
\item { WMS}. Para obtener imágenes de mapas.
\item { WCS}. Para obtener y consultar coberturas.
\item { WFS}. Para obtener y editar entidades geográficas y sus atributos asociados.
\item { WPS}. Para servicios de procesos remotos.
\item { GML}. Para almacenamiento de información geográfica.
\item { CSW}. Para consultas en catálogos.
\end{itemize}
